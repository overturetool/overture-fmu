def releaseCopy='''#/bin/bash

cd target/checkout

echo Detecting current version

version=`mvn -N org.apache.maven.plugins:maven-help-plugin:2.1.1:evaluate -Dexpression=project.version -DDmaven.repo.local=.repository/ | grep -v '\['`

cp "core/fmu-import-export/target/fmu-import-export-${version}-jar-with-dependencies.jar" ide/repository/target/repository/fmu-import-export.jar

cd ide/repository/target/repository
zip -r ../p2.zip .
mv ../p2.zip .
'''
