...
r = requests.get("https://api.github.com/repos/overturetool/overture-fmu/milestones?state=closed", headers=header)
...
#    writeMilestone(file,version,milestone['url'],milestone['due_on'],None)
    ...
    ri = requests.get("http://api.github.com/repos/overturetool/overture-fmu/issues?state=all&milestone="+str(milestone['number']), headers=header)
